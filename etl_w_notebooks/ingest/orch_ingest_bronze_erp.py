...
for i in range(1, 11, 1):
  ...
  bench_r.append({"concurrency": i, "result": r_raw})

# COMMAND ----------

new_bench_r = []
for r in bench_r:
  temp_res = [json.loads(j) for j in r['result']]
  for k in temp_res:
    k['concurrency'] = r['concurrency']
    new_bench_r.append(k)

# COMMAND ----------

r_df = spark.createDataFrame(new_bench_r)
(
  r_df
  .select(
    F.col('target'), F.col('concurrency'),
